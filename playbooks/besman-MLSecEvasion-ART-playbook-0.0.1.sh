...
function __besman_init() {
    ...
    export ASSESSMENT_TOOL_NAME="ART"
    export ASSESSMENT_TOOL_TYPE="Aadversarial Robustness"
    export ASSESSMENT_TOOL_VERSION="1.20.1"
    export ASSESSMENT_TOOL_PLAYBOOK="besman-MLSecEvasion-$ASSESSMENT_TOOL_NAME-playbook-0.0.1.sh"
    
    local steps_file_name="besman-MLSecEvasion-$ASSESSMENT_TOOL_NAME-steps-0.0.1.ipynb"
    ...
    if [[ $flag == true ]]; then
        ...
        export ART_EVASION_TEST_REPORT_PATH="$BESMAN_ASSESSMENT_DATASTORE_DIR/models/$BESMAN_ARTIFACT_NAME/fuzz-test/evasion"
        export DETAILED_REPORT_PATH="$ART_EVASION_TEST_REPORT_PATH/VulnerabilityReport.json"
        mkdir -p "$ART_EVASION_TEST_REPORT_PATH"
